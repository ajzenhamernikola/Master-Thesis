...
from dgl.nn.pytorch import GraphConv, SumPooling, MaxPooling, AvgPooling, SortPooling
...
class Regressor(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, hidden_layers: list, conv_channels: list,
                 conv_channels_sizes: list, activation: str, activation_params: dict, dropout_p: float, k: int,
                 pooling="avg"):
        super(Regressor, self).__init__()

        hidden_features_width = sum(hidden_layers)
        ...
        num_conv_channels = len(conv_channels)
        if num_conv_channels < 1:
            raise ValueError(f"You must have at least one convolutional layer. You passed the following array for " +
                             f"channels: {conv_channels}")
        num_conv_channels_sizes = len(conv_channels_sizes)
        if num_conv_channels_sizes != num_conv_channels:
            raise ValueError(f"The number of convolution channels and sizes is not equal: {num_conv_channels} != " +
                             f"{num_conv_channels_sizes}")
        ...
        for i in range(num_layers-1):
            self.hidden_layers.append(GraphConv(hidden_layers[i], hidden_layers[i + 1]))

        # Sort pooling
        self.sort_pool = SortPooling(k)

        # CNN layers
        self.conv_layers = nn.ModuleList()
        self.conv_layers.append(nn.Conv1d(1, conv_channels[0], hidden_features_width, hidden_features_width))
        #self.conv_layers.append(nn.Conv1d(1, conv_channels[0], hidden_layers[-1], hidden_layers[-1]))
        for i in range(num_conv_channels - 1):
            self.conv_layers.append(nn.MaxPool1d(2, 2))
            self.conv_layers.append(nn.Conv1d(conv_channels[i], conv_channels[i + 1], conv_channels_sizes[i], 1))
        self.conv_layers.append(nn.MaxPool1d(2, 2))

        # Batch normalization
        self.bn_layers = nn.ModuleList()
        self.bn_layers.append(nn.BatchNorm1d(1))
        for i, channel in enumerate(conv_channels):
            if i < len(conv_channels) - 1:
                self.bn_layers.append(nn.BatchNorm1d(channel))

        # MLP layer
        final_size = k
        for i, channel in enumerate(conv_channels_sizes):
            final_size = int(final_size / 2) if final_size % 2 == 0 else int((final_size - 1) / 2)
            if i != len(conv_channels_sizes) - 1:
                final_size -= channel - 1

        self.linear1 = nn.Linear(final_size * conv_channels[-1], final_size)
        self.linear2 = nn.Linear(final_size, output_dim)

        # Dropout
        self.dropout = nn.Dropout(p=dropout_p)

    def forward(self, g: dgl.DGLGraph):
        # Save the embedded features for sort pooling
        embedded = None
        ...
        for i, layer in enumerate(self.hidden_layers):
            ...
            if embedded is None:
                ...
                embedded = torch.cat([embedded, h], 1)

        # Perform pooling over all nodes in each graph in every layer
        embedded = self.sort_pool(g, embedded)

        # Reshape for CNN
        embedded = embedded.reshape((embedded.shape[0], 1, -1))

        # Perform CNN over embedded values
        for i, layer in enumerate(self.conv_layers):
            if i % 2 == 0:
                # embedded = self.dropout(embedded)
                embedded = self.bn_layers[i // 2](embedded)
                embedded = self.activation(layer(embedded))
            else:
                embedded = layer(embedded)

        # Return the predicted data in MLP
        embedded = embedded.reshape((embedded.shape[0], -1))
        embedded = self.activation(self.linear1(embedded))
        return self.linear2(embedded)
...
def train(train_device, test_device):
    ...
    conv_channels_str = '-'.join(list(map(lambda x: str(x), conv_channels)))
    conv_channels_sizes_str = '-'.join(list(map(lambda x: str(x), conv_channels_sizes)))
    activation_params_str = ''
    for key in activation_params.keys():
        activation_params_str += key + "+" + str(activation_params[key]) + ','

    mfn = f"dgcnn_{input_dim}_{output_dim}_" + \
          f"{hidden_layers_str}_{conv_channels_str}_" + \
          f"{conv_channels_sizes_str}_{k}_{activation}_{activation_params_str}_" + \
    ...
    log.log_line(f"Convolution net channels: {conv_channels}")
    log.log_line(f"Convolution net channels' sizes: {conv_channels_sizes}")
    log.log_line(f"k: {k}")
