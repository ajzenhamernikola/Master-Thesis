...
from dgl.nn.pytorch import GATConv, SumPooling, MaxPooling, AvgPooling
...
class GAT(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, num_heads, pooling="avg"):
        ...
        self.linear1 = nn.Linear(hidden_dim*num_heads, hidden_dim*num_heads)
    ...
    def forward(self, g: dgl.DGLGraph):
        ...
        h = self.pool(g, self.layer2(g, h).mean(1))
...
def train(model_output_dir, model, trainset, valset, trainvalset, train_device, test_device):
    ...
    print(f"GAT layer hidden dimension: {hidden_dim}")
    print(f"Number of heads in GAT: {num_heads}")
    ...
    overwrite = True
    retrain = False
    model_path = os.path.join(model_root, "best_GAT_model")
    ...
    if model_exists:
        ...
        predictor = GAT(in_dim=input_dim,
    ...
    if not retrain:
        ...
        while True:
            ...
            time_start = timer()
            train_loss = train_one_epoch(data_loader_train, train_num_of_batches, loss_func, predictor, optimizer,
                                         train_device)
            ...
            time_start = timer()
            val_loss, r2_score_val_avg, rmse_score_val_avg = validate_one_epoch(model_root, current_epoch, predictor,
                                                                                loss_func, data_loader_val,
                                                                                val_num_of_batches, train_device,
                                                                                test_device)
            time_elapsed = timer() - time_start
            ...
            torch.save([predictor, train_losses, val_losses, r2_scores, rmse_scores, train_times, val_times, []],
                       model_path)
    ...
    if retrain:
        ...
        while True:
            ...
            # Serialize model for later usage
            torch.save(
                [predictor, train_losses, val_losses, r2_scores, rmse_scores, train_times, val_times, retrain_times],
                model_path)
...
def test(model_output, model, testset, predict_device, test_device):
    test_batch_size = 1
    data_loader_test = DataLoader(testset, batch_size=test_batch_size, shuffle=False,
                                  collate_fn=collate(predict_device))
    test_num_of_batches = int(np.ceil(testset.__len__() / test_batch_size))

    # Load the model
    data = torch.load(os.path.join(model_output, model, "best_GAT_model"))
    predictor = data[0]
    ...
    png_file = os.path.join(model_output, model, "GAT.png")
    ...
    plt.plot([best_val_loss, best_val_loss], [0, highest_val_loss], color="green", linestyle="solid",
             label="best epoch")
    plt.ylim(0, 5)
    plt.legend()

    plt.tight_layout()
    plt.savefig(os.path.join(model_output, model, "GAT_losses.png"))
    plt.close()
